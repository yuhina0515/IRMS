...
import { installIrmsStub, mockIrmsApiModule, type IrmsStub } from '../test/irmsApiStub'

// platform/irmsApi 換成 stub(node project 沒有 setup.ts,每個需要它的檔案自己裝)。
vi.mock('@renderer/platform/irmsApi', () => mockIrmsApiModule)

// bluetoothService 是模組單例,建構時就會呼叫 @tauri-apps/api/event 的 listen()
// 訂閱 'ble:connection'/'ble:packet'——測試環境沒有真正的 Tauri runtime 注入
// window.__TAURI_INTERNALS__,不 mock 掉這裡 import 到 bluetoothService 時就會炸
// (registerEventListeners 是 async,炸出來的是 unhandled rejection)。這個檔案
// 只透過 beginSimulated/ingest/endSimulated 走模擬鏈路,不需要真的驅動事件,
// 所以只要 listen() 能正常 resolve、不去真的呼叫 invoke 就夠了。
