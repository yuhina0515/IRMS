...
export function installIrmsStub(overrides: IrmsStubOverrides = {}): IrmsStub {
  ...
  const defaults = {
    ...
    firmware: {
      ...
      checkLatest: async (_beta: boolean) => ({ version: '0.0.0', size: 0, notes: '', appCompatible: true }),
      downloadLatest: async (_version: string, _beta: boolean) => ({ path: '', size: 0, md5: '', data: new Uint8Array() }),
