...
// platform/irmsApi 換成 stub,理由見 irmsApiStub.ts 檔頭(node project 沒有
// setup.ts,每個需要它的檔案自己裝)。
vi.mock('@renderer/platform/irmsApi', () => mockIrmsApiModule)

// bluetoothService 建構時就會呼叫 @tauri-apps/api/event 的 listen() 訂閱
// 'ble:connection'/'ble:packet',測試環境沒有真正的 Tauri runtime,不 mock 掉
// import 到 bluetoothService 就會炸。這個檔案全程走 beginSimulated/ingest 模擬
// 鏈路(bluetoothService.send 被 spy 掉,從不真的呼叫 invoke),所以只要 listen()
// 能正常 resolve 就夠了。
