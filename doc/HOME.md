...
- **2026-09-24 test telemetry upload (branch `feat/telemetry-upload`)**: opt-in (default OFF)
  Settings panel uploads raw BLE packets, connection/OTA/Session events and app logs to
  `https://hina-tw.ddns.net/irms-api/` (collector in `IRMS_Telemetry/`, Docker on hina-server).
  Ingest token is typed in Settings, never committed. Live upload verified from the Rust client;
  GUI panel not yet exercised in a running app. See
...
- **2026-09-15 校正貼裝根因修復／v1.2.0-beta.10**
  ...
  (非外側)是校正長期不準的根因之一,裁決不強制改貼外側(外側無骨性標記、可能更不穩定)。
  已發布:精靈步驟1加小腿貼裝位置圖示(釘脛骨前緣)、步驟6預覽加外展殘留耦合提示
  (`couplingWarning`,非阻斷)。大腿外展步驟**維持選配**(強制會犧牲既有的單腳站立
  可及性設計,修正了裁決原文這一點);小腿維持即時預覽人工確認為主力。README §2.1
  與真機驗證閘門維持 2026-09-12 會議的決定不變,大腿圖示標記點同樣待真機驗證。
  「模組管理介面」未納入本次發版——09-11 會議裁決那是暫緩項,spike 本身尚未執行。
...
- **2026-09-15 Tauri v1.2.0-beta.8 Desktop Workstation UI 重構**([[log_20260915_beta8_desktop_workstation_redesign|日誌]]):
  ...
  285 前端 tests、60 Rust tests、build、rustfmt 與 Clippy 全綠，已發布 `v1.2.0-beta.8`
  並更新 `beta-latest`。
...
- **2026-09-11 動態模組系統裁決會議**([[log_20260911_meeting_dynamic_module_system|會議記錄]]):
  三方裁決「App 端執行期動態模組系統」該不該現在啟動,結論是拆成兩條互不依賴的軌道——
  **校正精靈重新設計現在就走傳統非模組化路線**(不等模組系統),**模組系統只做一次限定
...
- **⏸ 2026-09-14 beta.5 後續回報,診斷中斷待續**
  ([[log_20260914_beta5_followup_reports_and_open_diagnostics|完整日誌]]):使用者手動
  升級到 `1.2.0-beta.5` 後回報三個問題。視窗控制鈕偏移**澄清為從未修過**(09-11 已知
  限制,非回歸)。更新檢查與選單收合兩項的精確重現步驟**使用者尚未回覆**——已確認使用者
  是用手動安裝檔升級,還沒實際透過 App 內建「立即檢查更新」按鈕驗證過 beta.5 的修法,
  選單「展開」的回報也未確認是否真的測過「收合→關閉→重開」這個持久化才驗證得到的
...
- **2026-09-15 完整測試一輪 app**
  ([[log_20260915_full_app_test_pass|完整日誌]]):自動化檢查(typecheck/283 條測試/cargo
  check)全過;實機啟動後用 UI Automation + 真實硬體滑鼠事件逐項驗證,確認 09-14 懸置的
  兩個 beta.5 回報(更新檢查狀態文字、側邊欄收合持久化,含完整 `Stop-Process` 關閉重開
