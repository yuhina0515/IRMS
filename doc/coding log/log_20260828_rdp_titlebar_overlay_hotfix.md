...
自動化測試當下發現一個方法論疑慮:對這台機器截圖時一度抓到使用者另一台筆電
透過 RDP 連進來玩《英雄聯盟》的畫面,顯示這台機器同時有多個並發 RDP/主控台
session——沒辦法保證自動化測試的 4 個 process 真的跑在使用者測試 IRMS 的
那個 session 裡。因此請使用者在他自己那個 session 上親眼確認,取得的截圖顯示
v1.0.4 **在使用者實際的 RDP 連線上正常運作**:標準視窗標題列(非無邊框滿版,
符合 `IS_RDP_SESSION` 分支的預期行為)、Dashboard 畫面正確渲染(Guided
Monitoring、Squat 動作、目標角度量表、Connect Device 按鈕皆正常)。此前的
自動化壓力測試結果因此補上使用者端的真人確認,不再只依賴可能抓錯 session 的
本機截圖。
