...
- [ ] **App 端執行期動態模組系統(2026-09-10 裁決:方向確立,細節待 Phase 4 到位後裁決會議
      展開)**:起點是「移除校正/手動校正功能,改用 OTA 讓開發時定義好的值推給裝置」,經釐清
      ...
      - **模組分兩層**:基本模組安裝時自動部署、不可被使用者停用或卸載;其餘模組使用者
        可自由安裝/卸載/停用。「其餘模組」來源**暫定第一方**(自己開發、放進官方發布
        管道)——使用者尚未想清楚要不要開放第三方 plugin marketplace,這只是先拿來繼續
        規劃用的暫定假設,不是拍板決定,開裁決會議時要重新確認。
      ...
      - **動態載入程式碼的完整性驗證(2026-09-10 定案採用)**:這支 App 碰患者復健資料,
        ...
        真的動工設計時再展開。
      - **動工前置:比照 09-07 Tauri 遷移先例,先開一次三方裁決會議才動工(2026-09-10
        定案)**——不在 Phase 4 到位當下直接進入設計/實作。
      - **第一個要做的模組就是校正功能整套重練(2026-09-10 定案,取代原「校正體驗改為
        環境式/自動偵測」的構想——已判斷不採用,見下方獨立記錄)**:使用者實測回報現行
        校正架構「試了每個方向都認不出是哪一個環節算錯」,這正是
        [[log_20260908_meeting_single_imu_axis_orientation|09-08 會議]]已經診斷過的根因
        (兩顆感測器綁死一起、走同一套線性六步精靈,沒辦法單獨重測一顆有問題的感測器)。
        使用者裁決**不要**現在就單獨實作 09-08 裁決的修法(`recalibrateAxis(limb)`)當
        獨立修補,而是**等模組系統就緒後,把整套校正邏輯當成第一個模組從頭重新設計**。
        09-08 會議的診斷與裁決核心數學(原始向量旋轉 + `atan2`)**仍然是有效的參考起點,
        不是被推翻**,只是實作時機延後到模組系統就緒、且會在裁決會議上重新確認是否原封
        採用。**開發階段可以並行嘗試多種校正演算法/流程方向,正式發布只保留驗證後最準的
        一套**,不是要做成讓使用者自選校正方式的功能。
      尚未展開的開放問題:模組 manifest/版本比對格式、Settings UI 要怎麼呈現「模組」給
      使用者選、第一方/第三方的最終定案。**技術驗證(先做小規模 spike 確認真動態載入可行)
      已於 2026-09-10 明確暫緩,先不實作。尚未開始設計,不佔用桌面 backlog 的既有工作,
      整體仍卡在 Phase 4 這個依賴閘門上。**
