...
* **只實作,不設計**：Gemini 給出具體規格(mockup、色票、間距數值、排版決策)時,直接落地
  實作,不加入自己的美術判斷、不回頭要求使用者裁決是否採納(見
  [[log_20260902_ui_design_delegated_nav_cockpit|09-02 委任決定]]的既有慣例——這條規則是
  它的延伸,範圍從「Gemini 的決定照做」擴大到「這個角色本身不產生設計提案」)。
* **可驗證的事實仍要查證**：Gemini 聲稱的 WCAG 對比度、色彩數值計算等**可用程式驗證**的
  主張,查證程序不變(見 09-02 round2 日誌——曾經抓到 Gemini 自己聲稱的對比度數字算錯)。
  這條紀律針對的是「事實對不對」,不是「好不好看」,兩者不衝突。
* **實作細節的工程判斷不受影響**：Gemini 規格沒說清楚的純技術落地方式(元件拆分、效能、
  無障礙語意標籤如 `<table>` vs `<div>`)仍是這個角色的職責,不是設計權責的一部分。
* **提交給 Gemini 的素材維持既有慣例**：截圖 + README 說明包(`doc/gemini-handoff-*/`),
  明確劃出已定調範圍與待 Gemini 判斷範圍,交還使用者帶去外部工具操作(人工,不接 API)。
