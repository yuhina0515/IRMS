...
**(2026-09-24 起生效,取代 2026-09-11 版)設計稿可由 Claude Code 或 GPT 6 Astra 產出,
使用者為最終裁決者。** Gemini 因訂閱到期、09-14 brief 從未取得回覆而退出設計角色(使用者
2026-09-24 裁定,見 [[log_20260924_ui_redesign_plan|UI 改版計畫日誌]])。

* **設計提案必須是可落地的規格**:色票(hex)、字級/間距/圓角數值、動效時長與曲線、每個
  畫面狀態的構圖——不接受只有形容詞的提案。現行設計規格見 [[UI_DESIGN_LANGUAGE_V2]]。
* **可驗證的事實仍要查證,不論設計出自誰**:WCAG 對比度、色彩數值計算等可用程式驗證的主張,
  一律用腳本實算(`doc/design-v2/contrast_check.py`),不採信任何一方(包括自己)的口頭數字
  (09-02 round2 曾抓到設計方自己聲稱的對比度算錯)。
* **交叉審閱**:一方產出的設計可交由另一方審閱;兩方意見衝突且無法以可驗證事實裁定時,
  交使用者決定,不由實作者自行擇一。
* **實作細節的工程判斷不受影響**:規格未說清楚的純技術落地方式(元件拆分、效能、無障礙語意
  標籤如 `<table>` vs `<div>`)仍是實作角色的職責。
* **歷史**:2026-09-11~09-24 期間美術判斷全權委任 Gemini(`doc/gemini-handoff-*/` 為當時的
  交接素材),該期間的設計決定保留在 coding log,不追溯改寫。

---

## 1.2 多 AI 代理協作 (Multi-Agent Collaboration)

**（2026-09-14 起生效）** 使用者這台機器上同時裝了多個 agentic CLI 協同開發此專案:
Claude Code(本檔案的主要對象)、Gemini/Antigravity CLI(見 §1.1,設計權責)、以及新上線的
**Codex CLI**。這些代理**沒有共享的即時 session**,彼此看不到對方正在做什麼——唯一的溝通
管道是這個 repo 本身:git 歷史、`doc/coding log/`、`doc/HOME.md`。

* **開工前先看歷史,不要假設自己是唯一在動這個 repo 的人**:動手前跑 `git status`/`git log`
  確認沒有另一個代理留下的未預期未提交變更(這本來就是既有的 git safety protocol,這裡只是
  明確點出「未預期的變更」也可能來自另一個 AI,不只是使用者自己手動改的)。
* **尊重既有決定,不要不明就裡地回退**:如果發現另一個代理做的變更看起來奇怪或跟自己的直覺
  不同,先去讀對應的 coding log 條目(commit message 或 `doc/coding log/` 通常會留下理由),
  理解原因後再判斷要不要調整——不要因為「風格跟我不一樣」就直接覆寫或回退,那等於丟掉
  另一個代理已經做過的分析。
* **透過 commit/coding log 溝通,不要留給使用者口頭轉達**:如果做的事會影響另一個代理接下來
  可能接手的工作(例如改了一個共用的設定慣例、否決了一個提案),清楚寫進 commit message 跟
  coding log,讓對方(不論是哪個代理接手)讀 log 就能還原上下文,不必依賴使用者記得轉述。
* **這不是設計權責(§1.1)的延伸**:§1.1 是「美術/UI 判斷全權交給 Gemini」的單向委任;這裡
  講的是工程協作時彼此謙讓、留痕跡的一般禮貌,兩者範疇不同,不要混為一談。
