...
>
> **狀態:所有決策已裁定(見 §0);UI 重建已完成於分支(見 [[log_20260924_ui_rebuild_v2]]),待驗收後合併與實機驗證。** 本檔是「怎麼做、按什麼順序做、怎麼證明做對了」的工程計畫;
> **不是**視覺設計規格。計畫中所有需要美術判斷的地方都標成 🎨 **設計輸入槽**,
> 內容由 [[UI_DESIGN_LANGUAGE_V2]] 提供(設計來源依 [[AI_CODING_RULES]] §1.1:Claude Code 或
> GPT 6 Astra,使用者最終裁決)。

---

## 0. 2026-09-24 使用者裁定(覆寫下文與之衝突之處)

| 項目 | 裁定 | 對計畫的影響 |
|---|---|---|
| D-1 設計來源 | Claude Code 或 GPT 6 Astra,使用者最終裁決 | 設計規格見 [[UI_DESIGN_LANGUAGE_V2]] |
| D-2 合併時程 | **(a) 驗收結束後才合併** | 全部改版工作留在 `claude/irms-ui-redesign-38fvnr`,驗收前不進 `main`、不發 beta |
| D-4 介面語言 | **語言檔分為英文與中文** | 新增 `src/i18n/`:`zh-TW.ts` 與 `en.ts` 兩份語言檔(型別強制兩者鍵值一致),設定頁可切換,預設繁中 |
| 改版方式 | **UI 推掉重建**,不採漸進式整理 | 原 P1「零視覺差異地基整理」取消。改為:保留邏輯層(`services/`、`store/`、`platform/`、`shared/`、`hooks/`、`src-tauri/`),UI 層(`views/`、`components/`、`styles/`)依設計規格重新撰寫;舊樣式表整份移除而非拆層 |

重建順序:i18n 層 → 新 token 與樣式表 → 外殼 + Dashboard(含 `dashboardState` 純函式與專注模式)→
動作/紀錄/設定/對話框/校準精靈 → CI、截圖、文件。§3「不可動的邊界」維持不變——重建的是呈現層,
判定、BLE、DB、校準數學、OTA、updater 一律不動;元件測試遇到文案改變時更新斷言,不刪測試。
合併進 `main` 時,先在合併前的 `main` 上打 `ui-beta8-archive` tag 封存舊 UI(沿用 09-01 拆除 Liquid Glass 時的慣例)。
...
### P1 — 地基整理(零視覺變化)〔2026-09-24 取消:改為整體重建,見 §0〕
...
| # | 問題 | 選項 | 影響 |
|---|---|---|---|
| **D-1** ✅ | 🎨 設計輸入由誰提供? | **已裁定(2026-09-24)**:Claude Code 或 GPT 6 Astra,使用者最終裁決;§1.1 已改寫。第一版設計見 [[UI_DESIGN_LANGUAGE_V2]] | P2–P5 解除阻塞 |
| **D-2** ✅ (a) | 合併時程 | (a) 驗收結束後才合併(建議)<br>(b) 驗收前先合併 P1(零視覺差異,風險低) | 驗收 build 的穩定性 |
| **D-3** | 是否需要「病患專注模式」(大字、只留量表/次數/警報) | 要 / 不要 / 交給設計方判斷 | P3 範圍 |
| **D-4** ✅ 中英雙語言檔 | 介面文字:全中文、全英文,或中英並列(目前三套混用) | — | P1 標籤表、測試斷言;與 OPTIMIZATION 的 i18n 項相關 |
