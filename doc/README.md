...
#### 5.1.1 排錯:插入電腦後序列埠清單沒有出現 ESP32

依可能性排序檢查(而非埠被佔用——那個情境見
[AI_CODING_RULES.md §2](AI_CODING_RULES.md#2-esp32-韌體開發規則-esp32-firmware-rules)
「序列埠衝突防範」):

1. **USB 線材是「純充電線」**:最常見原因。許多線材只接電源腳、未接資料腳
   (D+/D-),電腦端完全偵測不到裝置。換一條確定能傳資料的線(例如手機傳檔用的
   那條)再試。
2. **USB-to-Serial 驅動未安裝**:ESP32 開發板上通常另外焊了一顆 USB 轉序列
   晶片(常見為 Silicon Labs **CP2102/CP2104** 或 WCH **CH340/CH9102**),電腦需要
   該晶片的驅動才會列出序列埠,和 ESP32 本身無關。
   - Windows:開「裝置管理員」,若在「其他裝置」或「通用序列匯流排控制器」下
     看到帶黃色驚嘆號的未知裝置,代表驅動未裝。CP210x 驅動見
     [Silicon Labs 官網](https://www.silabs.com/developers/usb-to-uart-bridge-vcp-drivers),
     CH340 驅動見 [WCH 官網](https://www.wch.cn/downloads/CH341SER_EXE.html)。
   - macOS:較新晶片(CP2102N、CH9102)多已有系統簽署驅動;若灰色不動,檢查
     「系統設定 → 隱私權與安全性」底部是否有驅動被封鎖需要允許載入。
   - Linux:插入後執行 `dmesg | tail` 應會看到 `ttyUSB0` 或 `ttyACM0` 等字樣;
     若完全沒反應,多半是驅動或硬體問題。若裝置出現但 Arduino IDE
     仍列不出來/燒錄權限被拒,把使用者加入 `dialout` 群組
     (`sudo usermod -aG dialout $USER`,需登出重登)。部分發行版的 `brltty`
     套件會搶佔 CP210x/CH340 裝置,可嘗試移除或停用該服務排除。
3. **USB 埠或集線器問題**:換另一個 USB 埠(優先用主機板/筆電直出的埠,避開無
   供電/資料能力不足的集線器或延長線)。
4. **確認開發板本身有通電**:插上後觀察板上電源指示燈是否亮起;沒有亮通常是
   線材或供電問題,而非驅動問題。
5. 以上都排除後仍偵測不到,才需要懷疑開發板本身的 USB 轉序列晶片故障,換一片
   板子測試以排除硬體本身損壞。
