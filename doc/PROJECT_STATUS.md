...
硬體相關內容自該版起未變)——**⚠ 本檔尚未反映 09-10 起的 Tauri 遷移**(見下方警語)。

> ⚠ **2026-09-16 現況警語**:本檔全文(含下方「一句話現況」與 v1.0.5 版本號)寫於
> Tauri 遷移(`IRMS_App_Tauri`,09-10 起)之前,描述的是 Electron v2 世代的階段性狀態,
> **不代表現況**。現役實作、目前版本(`1.2.0-beta.10`)、已驗證/未驗證項目、真機驗收
> 缺口,請見 [[HOME|doc/HOME.md]]「目前狀態速記」(尤其 09-16 的狀態評估與任務計畫)與
> [`doc/coding log/`](coding%20log/)。本檔按原文保留作為 Electron v2 階段的歷史快照,
> 尚未依決策 D2「文件與程式衝突時以程式為準」重寫為現況文件——這件事本身就是
> DOC-01(見 09-16 任務計畫)待完成的項目。
