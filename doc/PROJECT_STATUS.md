...
| 自動化測試 | **144 tests / 14 files 全綠**;`npm run ci` = typecheck + test + build |
| DB schema | `user_version = 5`(遞增式 migration) |
...
| 風險 | 說明 |
|---|---|
| 🔴 **回饋與警報鏈未驗證** | 連線與校準已在實機驗證,但達標音、超限長鳴、斷線收尾這條鏈從未在真裝置上跑過——這正是判定引擎唯一會**主動對患者發出指令**的部分(issue [#3](https://github.com/yuhina0515/IRMS/issues/3)) |
