...
**2026-08-28**(issue #2 收尾:韌體重新燒錄+旋轉記錄完成並關閉;上一次全面更新為 2026-08-12)
...
桌面端功能完整、測試齊備、已發布安裝檔;硬體迴路的**前半段**(燒錄 → BLE 連線 → 校準 →
旋轉記錄)已於 2026-08-07 / 2026-08-28 在真裝置上驗證正常(issue #2 已關閉),**後半段**
(達標回饋 → 超限警報 → 斷線收尾)仍未驗證。裝置**目前仍在手邊**——下一步是直接拿
issue [#3](https://github.com/yuhina0515/IRMS/issues/3) 附的 ~30 分鐘 E2E 腳本實測,
把握這段時間窗。

| 項目 | 狀態 |
|---|---|
| 應用端版本 | **v1.0.1**(已發布 NSIS 安裝檔,未簽章) |
| 自動化測試 | **281 tests / 25 files 全綠**(雙 project:node 純邏輯 + dom 元件);`npm run ci` = typecheck + test + build |
| DB schema | `user_version = 7`(遞增式 migration) |
...
| 實機驗證 | **部分**:連線、校準精靈、快速歸零修正(2026-08-07)+ 桌上 ±180° 旋轉記錄(2026-08-28,issue #2 已關閉)皆已在真裝置上跑過並確認正常。**未驗證**:達標回饋、超限警報、斷線收尾這條完整 E2E 鏈(issue #3) |
...
| 風險 | 說明 |
|---|---|
| 🔴 **回饋與警報鏈未在實機驗證** | 2026-08-27 已為這條鏈建立**指令稽核測試**(有序 `CMD:` 斷言,涵蓋靜音自動重新武裝、`ERR:` 強制關閉、重連重新武裝),並因此抓到「達標 LED 從第一下卡亮到 Session 結束」這個臨床缺陷。**但那證明的是 App 送出正確的字串與順序,不證明它們真的驅動 GPIO 25/26**——issue [#3](https://github.com/yuhina0515/IRMS/issues/3) 仍完全開啟 |
| 🟢 **UI 已目視驗收**(2026-08-27) | 以 Playwright 拋棄式驅動腳本跑完整條示範模式路徑並逐張截圖比對:全域橫幅、Demo 確認對話框、即時量表反應、Actions 搜尋空狀態、**真的跑完一場 Session**、History 徽章、分析 modal 橫幅、CSV 檔名(`irms_DEMO_session_1.csv`)全數確認正確。腳本本身未進版控。重連進度軌道(需真實斷線事件)與校準精靈逐步畫面仍未展開驗收 |
...
| 🔴 **裝置取得零星** | 每次拿到裝置的時間窗都稀缺,**沒錄成可解讀資料的 session 等於白用一次時間窗**。issue #2 已完成;完整 E2E(issue [#3](https://github.com/yuhina0515/IRMS/issues/3))仍待進行 |
...
- **自動化**:144 tests / 14 files(判定狀態機邊界、封包解析、校準、環形角度數學、
  guidance、smoothing、UI 節流、LTTB、validation、migration 升級路徑、escapeStack)。
...
- **打包**:v1.0.0 / v1.0.1 皆實測安裝檔可正常啟動、自訂圖示生效。
